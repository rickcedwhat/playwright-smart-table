 ...
 * CR opens every formal review with a line like "Actionable comments posted: N".
 * This is the reliable signal for whether a review has blocking issues —
 * nitpicks are NOT counted here (CR tallies them separately).
 *
 * @param {string|null|undefined} body
 * @returns {number} actionable comment count (0 if absent/unparseable)
 */
export function parseActionableCount(body) {
 ...
 * CR groups them under a "🧹 Nitpick comments (M)" summary header.
 * Used for surfacing a count only — nitpicks do not gate the merge.
 *
 * @param {string|null|undefined} body
 * @returns {number} nitpick count (0 if absent/unparseable)
 */
export function parseNitpickCount(body) {
